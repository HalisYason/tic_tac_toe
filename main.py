...
class app(tk.Tk):
    def __init__(self) -> None:
        ...
        dosya_menu.add_command(label="format", command=self.yeni_pencere2)
        ...
        self.koy = tk.Button(text="KOY",command=self.ekrana_koy,bg="lightgreen")
        self.koy.grid(row=7,column=1)

        self.kapat = tk.Button(text="PES",command=self.kapatt,bg="pink")
        self.kapat.grid(row=7,column=2)


        self.tekrar = tk.Button(text = "TEKRAR",command=self.tekrar_buton,bg="lightblue")
    ...
    def yeni_pencere2(self):
        # Yeni pencere oluştur
        yeni_pencere = tk.Toplevel(self.master)
        yeni_pencere.title("format")

        # Yeni pencere içeriğini ekle
        etiket = tk.Label(yeni_pencere, text="entrye girilmesi gereken format\n\nSATIR - SÜTUN",fg="green",bg="grey")
        etiket.pack(padx=20, pady=20)     
